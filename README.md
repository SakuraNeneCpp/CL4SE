...
CL4SE (Caps Lock for Safe Enter) は、メッセージ送信に使われがちな Enter をIME確定で押すことによる誤送信を減らすため、Caps LockにIME確定を割り当てるキーバインドツール。任意設定で、非変換中のCaps LockをShift+Enterによる改行にも割り当てられる。
...
v1.0.3以降は、固定配置したバイナリから次を実行する。

```sh
cl4se update
```

GitHubの最新安定版を確認し、実行中の版より新しい場合だけOSに対応する成果物をダウンロードする。`SHA256SUMS`との完全一致を検証してから、実行中のCL4SEを通常のクリーンアップ経路で停止し、同じパスのバイナリを置き換える。更新前に動作していた場合は更新後もバックグラウンドで再開する。置換に失敗した場合は旧バイナリへロールバックする。WindowsはPowerShellと`certutil`、macOSは`curl`と`shasum`、Linuxは`curl`と`sha256sum`を使用する。
...
| IMEオンだが非変換中 | `idle_action`。既定は何もしない。任意設定でShift+Enterを注入 |
| IMEオフ(状態を `No` と確定できる) | `idle_action`。既定は何もしない。任意設定でShift+Enterを注入 |
| IME状態不明 | 何もしない(設定にかかわらず安全側に倒す) |
...
- `idle_action = "shift_enter"` はオプトイン機能。既知のIME状態(`Yes` / `No`)かつトラッカーが非変換中の場合だけ、Shift down → Enter down → Enter up → Shift up の一連を注入する。既定の `none` では無効。
...
任意設定のShift+Enterは確定キーとは別の非変換時アクションであり、IME状態が `Yes` / `No` と判定でき、かつトラッカーが非変換中のときだけ注入する。`Unknown` では `idle_action` の値にかかわらず何も注入しない。Shift+Enterの意味はアプリ依存のため既定はOFFとし、利用するアプリで「送信せず改行」になることを確認したユーザーだけが有効化する。
...
| No | 非変換中(`refresh` 時にIdleへリセット) | `idle_action` |
| Unknown | (問わず) | 何もしない(Caps Lockのみ抑止) |

`idle_action` は `none`(何もしない)、`shift_enter`(Shift+Enter注入)、`capslock`(本来のCapsLockトグル)から選ぶ。`shift_enter` と `capslock` は `ime_active` が `Yes` / `No` と判定できる場合に限って実行し、`Unknown` では常に抑止する。
...
```toml
[general]
idle_action = "none"          # none | shift_enter | capslock (非変換中の動作。既定OFF)
...
安全改行を有効にするには `idle_action = "shift_enter"`、無効に戻すには `idle_action = "none"` としてCL4SEを再起動する。既存の設定ファイルは自動変更しないため、更新後も明示的に有効化するまでOFFのままになる。

通常は設定ファイルを直接編集せず、次のCLIを使用する。値を変更すると設定を保存し、実行中のCL4SEへ再起動を要求する。停止中の場合は次回起動時に反映される。

```sh
cl4se setting show
cl4se setting idle-action shift-enter  # 安全改行をON
cl4se setting idle-action none         # 安全改行をOFF
...
| `cl4se update` | GitHubの最新安定版をSHA-256検証後に同じパスへ導入。稼働中なら安全に停止・再開 |
| `cl4se setting show` | 現在の変更可能な設定と設定ファイルのパスを表示 |
| `cl4se setting idle-action <値>` | `none` / `shift-enter` / `capslock` を保存し、実行中なら安全に自動再起動 |
...
pub enum Decision { InjectCommitKey(CommitKey), InjectShiftEnter, PassThroughCapsLock, Suppress, Ignore }
...
| キー注入 | `CGEventCreateKeyboardEvent`(確定用kVK_Return = 36 / kVK_ANSI_M = 46 + Control、任意設定の安全改行用Shift + Returnの一連)→ `CGEventPost`。`CGEventSetIntegerValueField(kCGEventSourceUserData, マーカー)` で自イベント識別 |
...
| キー注入 | uinput 仮想キーボード(`evdev` クレートの VirtualDevice)で確定用KEY_ENTER / KEY_LEFTCTRL + KEY_M、任意設定の安全改行用KEY_LEFTSHIFT + KEY_ENTERの一連。自デバイス由来のイベントは監視対象から除外 |
...
- `idle_action = "shift_enter"` はShift+Enterを「送信せず改行」と扱うアプリ向け。異なるショートカットを割り当てたアプリでは意図しない動作になり得るため既定OFF。利用アプリごとにREADME §5.5 T10を確認する。
